# Figures for Dissociality scores
...
# Load data 
d <- read.csv(file.path(data_raw_csv, "dissociality_scores.csv")) %>%
  as.data.frame()
...
pX = "Group"
pY = "Mean score"
saveName = file.path(figures, 'dissociality.png')
...
fig <- ggplot(d, aes(x = group, y = yData, fill = group)) + 
  ...
  scale_y_continuous(breaks = seq(12, 60, 6), limits = c(12, 60)) +
  theme_minimal() +
  labs(title = pTitle, 
       x = pX, 
       y = pY) + 
